...
# Construct Atlas classpath using jars from hook/pinot/atlas-pinot-plugin-impl/ directory.
for i in "${BASEDIR}/hook/pinot/atlas-pinot-plugin-impl/"*.jar; do
...
LOGFILE="$ATLAS_LOG_DIR/import-pinot.log"
...
JAVA_PROPERTIES="$ATLAS_OPTS -Datlas.log.dir=$ATLAS_LOG_DIR -Datlas.log.file=import-pinot.log
-Dlog4j.configuration=atlas-pinot-import-log4j.xml"
shift
...
"${JAVA_BIN}" ${JAVA_PROPERTIES} -cp "${CP}" org.apache.atlas.pinot.bridge.PinotBridge $allargs

RETVAL=$?
[ $RETVAL -eq 0 ] && echo Pinot Data Model imported successfully!!!
[ $RETVAL -ne 0 ] && echo Failed to import Pinot Data Model!!!
