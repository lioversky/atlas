...
define(['require',
    'backbone',
    'hbs!tmpl/quality/QualityLayoutView_tmpl',
    ...
    'platform',
    'jquery-ui'
], function(require, Backbone, QualityLayoutView, VLineageList, VEntity, Utils, LineageUtils, dagreD3, d3Tip, Enums, UrlLinks, Globals, CommonViewFunction, platform) {
    'use strict';
    ...
    var LineageLayoutView = Backbone.Marionette.LayoutView.extend(
        /** @lends LineageLayoutView */
        {
            ...
            ui: {
                ...
                qualityFullscreenToggler: '[data-id="fullScreen-toggler"]',
                filterBox: '.filter-box',
                searchBox: '.search-box',
                settingBox: '.setting-box',
                qualityTypeSearch: '[data-id="typeSearch"]',
            ...
            onCheckUnwantedEntity: function(e) {
                var data = $.extend(true, {}, this.qualityData);
                //this.fromToNodeData = {};
                this.initializeGraph();
            ...
            onClickNodeToggler: function(options) {
                this.toggleBoxPanel({ el: this.$('.quality-node-detail'), nodeDetailToggler: true });
            },
                ...
                this.collection.getLineage(this.guid, {
                    ...
                    success: function(data) {
                        ...
                        if (data.relations.length) {
                            that.qualityData = $.extend(true, {}, data);
            ...
            noLineage: function() {
                ...
                this.$('svg').html('<text x="50%" y="50%" alignment-baseline="middle" text-anchor="middle">No quality data found</text>');
            ...
            createGraph: function() {
                ...
                d3.selectAll(this.$('.qualityZoomButton')).on('click', zoomClick);
                var tooltip = d3Tip()
                    .attr('class', 'd3-tip')
                    .offset([10, 0])
                    .html(function(d) {
                ...
                svgGroup.selectAll("g.edgePath path.path").on('click', function(d) {
                    var data = { obj: _.find(that.qualityData.relations, { "fromEntityId": d.v, "toEntityId": d.w }) };
                    if (data.obj) {
            ...
            renderLineageTypeSearch: function() {
                var that = this,
                    qualityData = $.extend(true, {}, this.qualityData),
                    data = [],
                    typeStr = '<option></option>';
                if (!_.isEmpty(qualityData)) {
                    _.each(qualityData.guidEntityMap, function(obj, index) {
                ...
                this.ui.qualityTypeSearch.html(typeStr);
                this.initilizequalityTypeSearch();
            },
            initilizequalityTypeSearch: function() {
                var that = this;
                that.ui.qualityTypeSearch.select2({
                    closeOnSelect: true,
                ...
                if (that.searchNodeObj.selectedNode) {
                    that.ui.qualityTypeSearch.val(that.searchNodeObj.selectedNode);
            ...
            onClickSaveSvg: function(e, a) {
                ...
                if (loaderTargetDiv.hasClass('fa-refresh')) {
                    Utils.notifyWarn({
                        content: "Please wait while the quality gets downloaded"
                    });
